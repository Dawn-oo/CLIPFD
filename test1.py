...
def main():
    print("=" * 80)
    print("开始加载消融实验测试参数配置...")
    opt = TestOptions().parse(print_options=False)
    opt.fusion_mode = "local_only"
    ...
    save_dir = project_root / "test_result" / opt.name
    ...
    reporter.save_best_report(
        ...
        best_metric_value=float(test_metrics["macro_auc"]) if "macro_auc" in test_metrics else float(test_metrics["loss"]),
    ...
    if opt.save_predictions:
        save_prediction_csv(
            save_path=save_dir / opt.prediction_csv_name,
